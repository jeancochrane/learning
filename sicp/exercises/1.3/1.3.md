...
raises its argument to the `2n`th power. Write a procedure that takes as inputs
...
search for `y -> x/y^3` converge. On the other hand, if we average damp twice (i.e.,
use the average damp of the average damp of `y -> x/y^3`) the fixed-point search does
converge. Do some experiments to determine how many average damps are required
to compute `n`th roots as a fixed-point search based upon repeated average
damping of `y -> x/y^(n-1)`. Use this to implement a simple procedure for computing `n`th
